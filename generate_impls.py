...
for i in range(0,8):
    ...
    tuple_ty = "()" if i == 0 else ("(_A,)" if i == 1 else ("(" + ", ".join(generics) + ")"))
    ...
    join_variant = "" if i == 0 else ("(" + ", ".join(["Either<_P" + chr(ord('A') + j) + ", _" + chr(ord('A') + j) + ">" for j in range(i)]) + ")")
    join_generics = "" if i == 0 else ("<" + ", ".join(generics + ["_P" + chr(ord('A') + j) for j in range(i)]) + ">")
    awaitable_generics = ["_P" + chr(ord('A') + j) for j in range(i)]
    awaitable_tuple_ty = "()" if i == 0 else ("(_PA,)" if i == 1 else ("(" + ", ".join(awaitable_generics) + ")"))
    where_poll = "" if i == 0 else (" where " + ", ".join("_P" + chr(ord('A') + j) + ": Poll<::Result=_" + chr(ord('A') + j) + ">" for j in range(i)))
