...
i8* gc_remap_find(i8* ptr) {
    ...
    DEBUG_PRINTF("REMAP FAIL: %p.\n", ptr);
    return ptr;
}

void gc_remap_insert(i8* from, i8* to) {
...
i1 gc_remap_and_unmark(i8** ptr) {
  ...
  i8* to = *ptr = gc_remap_find(*ptr - sizeof(i32) - sizeof(i16)) + sizeof(i32) + sizeof(i16);
...
void* gc_alloc_block(i64 size, i16 type, i8* cheshire_stack_root) {
    ...
    DEBUG_PRINTF("Allocating size %"PRId64" ID=%"PRId16"\n", size, type);
...
void gc_walk(i8* cheshire_stack_root, GC_CALLBACK) {
    ...
    while (frame) {
        ...
        for (int i = 0; i < frame->numSlots; i++) {
            ...
            if(ptrSlot.kind >= 0) {
                ...
                DEBUG_PRINTF("unexpected derived pointer\n");
                exit(-1);
            }
...
NOINLINE void gc(i8* cheshire_stack_root) {
    // Mark
    DEBUG_PRINTF(" -- MARK --\n");
    gc_walk(cheshire_stack_root, gc_mark);

    // Compact
    DEBUG_PRINTF(" -- COMPACT --\n");
    DEBUG_PRINTF("Mem pool begins at %"PRId64" long.\n", (i64) (GC_END - GC_BEGIN));
    i8* from = GC_BEGIN, * to = GC_BEGIN;
    ...
    while (from < GC_END) {
        ...
        if (marked) {
            if (from != to) {
                gc_remap_insert(from, to);
                memmove(to, from, block_size);
            }

            to += block_size;
        } else {
            i16 id = *(i16*) (from + sizeof(i32));
    ...
    GC_END = to;
    DEBUG_PRINTF("Mem pool is now %"PRId64" long.\n", (i64) (GC_END - GC_BEGIN));

    // Remap the pointers
    DEBUG_PRINTF(" -- UNMARK-REMAP --\n");
    gc_walk(cheshire_stack_root, gc_remap_and_unmark);

    // Free the list.
    DEBUG_PRINTF(" -- FREE RMAP TABLE --\n\n\n");
    gc_remap_free();
